...
selfcheck() {
  ...
  if [ -f "$skill_real" ]; then
    ...
    echo "  ⚠️  真源缺失：.cursor/skills/gtd-harness/SKILL.md（请先完成 Phase 2）"
  fi
  # 三平台符号链接入口
  local entry
  for entry in .claude/skills .agent/skills .agents/skills; do
    if [ -e "$VAULT_ROOT/$entry/gtd-harness/SKILL.md" ]; then
      ...
      echo "  ⚠️  入口未解析：$entry/gtd-harness（符号链接或 SKILL.md 缺失）"
    fi
  done
  # 复用仓库既有的目录一致性检查（若存在）
...
echo ""
echo "  下一步：跑一次捕捉 —— /gtd-capture（cc）或读 .cursor/skills/gtd-harness/capture/SKILL.md"
echo "  看全景仪表盘：bash .cursor/skills/gtd-harness/scripts/gtd_status.sh"
