...
└── 是 → 下一步具体的物理动作是什么？
    ...
    └── 自己做、>2 分钟  →
         ├── 特定时间/日才做  → hard landscape（日历）：真实 GCal 可达则**起草一笔可检查的写入提案、显式确认后写入**；不可达则记 calendar.md 兜底
...
3. **落位**：按决策树把该项追加到目标清单对应分组末尾（格式见各清单模板）。
   ...
   - **特定时间事 → 日历（hard landscape）**：单一日历铁律——真实 GCal 可达时它是唯一真源，**不抄进 calendar.md**。写入 GCal 是高后果操作，按 `references/capability-map.md` 写入确认契约：先**起草可检查提案**（确切事项+日期/时间+涉及人+来源+确认问句），用户**显式确认**后才写；tool 返回成功才报「已写入」，失败/不可达则记 calendar.md 兜底并如实说明，**绝不谎报**。
