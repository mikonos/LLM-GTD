...
1. **运行初始化脚本**（幂等、零破坏，已存在的清单跳过不覆盖）：
   ```
   bash scripts/gtd_init.sh
   ```
   插件模式下，脚本以当前 Codex workspace 为状态根；旧 `.cursor` 安装模式下，脚本仍会定位到安装它的 vault。它会建齐 `memory/gtd/` 八清单（含情境骨架与 Horizons 模板）并自检适配层。
...
- [ ] Codex 插件模式下不安装全局 slash prompt；旧安装模式下 `~/.codex/prompts/gtd*.md` 齐全
- [ ] 重跑只跳过、不覆盖（幂等）
