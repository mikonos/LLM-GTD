...
4. **projects 里塞完整任务树** → 只保留当前可并行的 next-action / waiting-for block links；里程碑、依赖和任务树进 `reference.md` 或项目文档。

## Obsidian 链接规则

- 指向 `memory/gtd/` 某个文件内标题：用 `[[文件名#标题|标题]]`，例如 `[[reference#条目名|条目名]]`。
- 指向 `next-actions.md` / `waiting-for.md` 某条具体行动：优先用 block link，例如 action 行末加 `^na-short-id-YYYYMMDD`，project 里写一个或多个 `[[next-actions#^na-short-id-YYYYMMDD|具体下一步行动]]（@电脑）`；等待项写 `[[waiting-for#^wf-short-id-YYYYMMDD|等待某人交付什么]]（等待）`。
- 指向真实独立文件：才用裸 `[[文件名]]`。
