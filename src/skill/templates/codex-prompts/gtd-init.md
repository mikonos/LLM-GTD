...
本命令：读 `rtk cat .cursor/skills/gtd-harness/init/SKILL.md` 并执行。默认 `rtk bash .cursor/skills/gtd-harness/scripts/gtd_init.sh`（幂等建 memory/gtd/ 八清单 + 安装/刷新 Codex /gtd* prompts + 自检）；参数含 --import-legacy 则附带导入旧 open loops（旧文件只读）；--status 只自检、不写文件。
