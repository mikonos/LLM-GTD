...
本命令：读 `rtk cat .cursor/skills/gtd-harness/clarify/SKILL.md` 并执行。对 inbox 每项跑决策树（可行动→2分钟/委派/推迟/项目；不可行动→垃圾/someday/reference），落到正确清单（next-actions 带情境、>1步立 project、知识类移交 ZK），落位后从 inbox 删除。特定时间事→真实 GCal 起草可检查提案+显式确认后写，不可达记 calendar.md 兜底。
