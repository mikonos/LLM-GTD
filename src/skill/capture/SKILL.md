---
name: gtd-capture
description: GTD harness 场景命令 · 收集。把输入零摩擦落盘 inbox，然后（默认）AI 自动 clarify 归位。GTD 第一+（自动）第二步。
parent: gtd-harness
---
...
4. **off-switch**：用户说「只捕捉 / 先别理 / just record」→ 纯落盘 inbox，不自动 clarify，留给之后 `/gtd-clarify`。
