...
本命令：读包导航后，按「六场景命令 routing」表判断用户意图，`cat` 对应子命令并执行。memory/gtd/ 不存在则先走 init。用户可以直接在 `/gtd` 后接自然语言，不必指定子命令；若像「Schedule coffee with Jack tomorrow afternoon at the Starbucks near my home」这类新输入/承诺，走 capture→clarify。意图确实模糊时再反问一句。
