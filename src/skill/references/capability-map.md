...
| 跑 dashboard / init 脚本 | Bash | 原生 terminal | `rtk bash .cursor/skills/gtd-harness/scripts/<x>.sh` |
| 调度每周回顾提醒 | `loop` skill / cron（`CronCreate`） | 手动节律 | launchd / cron |
| 反问澄清 | AskUserQuestion | 原生提问 | 文本提问 |
| **读取 hard landscape（日历）** | google_calendar MCP（list events，需 OAuth）| 原生 GCal 集成 | 你的 Codex 工作区 已接的 GCal 连接 / MCP |
| **写入日历事件（需确认）** | google_calendar MCP（create event）| 原生 | 你的 Codex 工作区 GCal 连接 |
...
**写入确认契约（沿用 你的 Codex 工作区 AGENTS.md：schedule change 需显式确认 + fail-closed）**：
...
1. **状态层零适配**：`memory/gtd/` 是纯 markdown 文件——文件就是文件，三平台读写同一份，无需翻译。多机经云同步同步（与既有 `open loops.md` 同一风险模型）。
